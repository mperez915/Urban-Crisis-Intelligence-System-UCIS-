...
CONFIG_POLL_INTERVAL = (
    1  # seconds between MongoDB config re-reads (fast pause response)
)
...
class SimulatorConfig:
    ...
    def update(self, cfg_doc: dict, scenario_doc: Optional[dict]):
        self.event_rate = max(
            1, min(int(cfg_doc.get("event_rate", self.event_rate)), 20)
        )
    ...
    def scenario_name(self) -> str:
        if self.active_scenario:
            return self.active_scenario.get(
                "name", self.active_scenario.get("scenario_id", "?")
            )
        return "default"
...
class EventSimulator:
    def __init__(self):
        ...
        logger.info(
            "Simulator initialised — config poll every %ds", CONFIG_POLL_INTERVAL
        )
    ...
    def connect_rabbitmq(self):
        ...
        logger.info(
            "Connected to RabbitMQ at %s:%s", self.rabbitmq_host, self.rabbitmq_port
        )
    ...
    def _poll_config(self):
        ...
        try:
            cfg_doc = (
                self.mongo.ucis_db.simulator_config.find_one({"_id": "main"}) or {}
            )

            scenario_doc = None
            sid = cfg_doc.get("active_scenario_id")
            if sid:
                scenario_doc = self.mongo.ucis_db.scenarios.find_one(
                    {"scenario_id": sid}
                )
            ...
            if (
                self.config.event_rate != old_rate
                or self.config.scenario_name != old_scenario
            ):
                logger.info(
                    ...
                    self.config.paused,
                    self.config.force_zone,
                    self.config.force_severity,
                )
    ...
    def publish_event(self, event: Dict[str, Any], domain: str) -> bool:
        ...
        for attempt in (1, 2):
            try:
                ...
                logger.warning(
                    "Publish attempt %d failed (%s); reconnecting…", attempt, e
                )
                try:
